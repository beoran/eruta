...
module Zori 
  class Mapeditor < Widget
    ...
    def on_key(sym, mod, text = nil)
      select sym 
      case Sisa::Key::L      # Add new Layer.
        puts "Map new layer: #{map.tw}, #{map.th}"
        map.new_layer(@map.tw, @map.th) 
      case Sisa::Key::C      # Copy selection.        
        @edit_select.copy(@map, @edit_layer)
      case  Sisa::Key::P # Paste selection. 
      ...
      case Sisa::Key::F10
        @done = true 
      case Sisa::Key::ESCAPE # Exit without save.
        done = Zori::Dialog.yesno('Quit without saving?')
      ...
      case Sisa::Key::KP_MULTIPLY
        @speed += 5 if @speed < 100
      case Sisa::Key::KP_DIVIDE
        @speed -= 5 if @speed > 5
      # Up to next layer        
      case Sisa::Key::KP_PLUS
        @edit_layer += 1 if @edit_layer  < (map.layer_count-1)
      # Down to previous layer   
      case Sisa::Key::KP_MINUS   
        @edit_layer -= 1 if @edit_layer  > 0
      case Sisa::Key::F2, Sisa::Key::F10
      # Save map
