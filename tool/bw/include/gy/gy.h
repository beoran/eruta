...
*
* More details are documented in gy.h
*
* Gy is copyright beoran@rubyforge.org 2010-2011. 
* Gy is released under the zlib license.  
* 
* @file gari.h
*
*/
...
/** GyRect models a rectangular box. */
struct GyRect_ {
  int x, y, w, h;
};

typedef struct GyRect_ GyRect;
...
/** Images. Images are visuals that can be dwawn to. */
struct GyImage_; 
typedef struct GyImage_ GyImage;

/** A screen. Not directly drawable. */
struct GyScreen_; 
...
struct GyEvent_ {
  ...
  uint16_t  button;
  int16_t   value;
  /* uint32_t  window; Perhaps later for multi-window support. */
};
...
* Return NULL if the evnt queue was empty, non-null if there was an event fetched. 
*/
...
/* Key symbols */
#define GYKEY_ESC   27
#define GYKEY_RIGHT 79
#define GYKEY_LEFT  80
#define GYKEY_DOWN  81 
#define GYKEY_UP    82
...
/** Allow joysticks to generate events or not. Call this before 
calling gyjoy_openall */
...
/** GyAffine is a 3x3 matrix with the last row (0,0,1) ommitted. */
struct GyAffine_;
typedef struct GyAffine_ GyAffine;
struct GyAffine_ {
  double x1, x2, x3;
  double y1, y2, y3;
};
...
/** Blit using an affine matrix transformation.*/
...
/** Blend modes to use  */
enum GyBlendMode_;
typedef enum GyBlendMode_ GyBlendMode;

enum GyBlendMode_ {
 GyBlendModeError = -1,
 GyBlendModeNone  = 0x00000000,
 GyBlendModeBlend = 0x00000001,
 GyBlendModeAdd   = 0x00000002,
 GyBlendModeMod   = 0x00000004
};


/** A Canvas is a rendering context that can be used to draw on in 
an accellerated way. In SDL it's called a Renderer, 
but that name is too long and doesn't sound nice. */
struct GyCanvas_; 
typedef struct GyCanvas_ GyCanvas;
...
/** Sets the view port of the canvas. The viewport modifies most drawing
operations so they are shifted with regards to the x, y of the viewport.
It clips any points that fall outside of it. */
GyCanvas * gycanvas_viewport_(GyCanvas * self, int x, int y, int w, int h);

/** Gets the view port of the canvas. The viewport modifies most drawing
operations so they are shifted with regards to the x, y of the viewport.
It clips any points that fall outside of it. */
GyCanvas * gycanvas_viewport(GyCanvas * self, int *x, int *y, int *w, int *h);

/** A Texture is an accelerated image that cannot be drawn to directly,
but that can be draw to a Canvas quickly. */
struct GyTexture_; 
typedef struct GyTexture_ GyTexture;

/** Generates a new texture for the given canvas from the given image. */
GyTexture * gytexture_new(GyCanvas * canvas, GyImage * image);

/** Frees a generated texture. */
void gytexture_free(GyTexture * self);

/** Queries a texture's parameters. */
GyTexture * gytexture_query(GyTexture * self, uint32_t * format, 
                            int * access, int * w, int * h);

/** Returns the width of the texture. Returns 0 on error. */
int gytexture_w(GyTexture * self); 

/** Returns the height of the texture. Returns 0 on error. */
int gytexture_h(GyTexture * self);

/** Returns the format of the texture. */
uint32_t gytexture_format(GyTexture * self);

/** Returns the access mode of the texture. */
int gytexture_access(GyTexture * self);

/** Sets the color modifications on the texture. */
int gytexture_color_(GyTexture * self, uint8_t r, uint8_t g, uint8_t b);

/** Gets the color modifications on the texture. */
int gytexture_color(GyTexture * self, uint8_t *r, uint8_t *g, uint8_t *b);

/** Sets the whole texture's alpha level. */
int gytexture_alpha_(GyTexture * self, uint8_t a);

/** Gets the whole texture's alpha level. */
...
/** Window handling. */
struct GyWindow_;
typedef struct GyWindow_ GyWindow;

/** Allocates a window structure and does nothing else. */
GyWindow * gywindow_alloc();

/** Frees a window structure, calling gywindow_done() if needed */
...
/** Updates the canvas with the drawing commands sent before. The window's 
...
/** Draws a box (the outline of a rectangle) on the canvas in the current
draw color. W and h are the offsets from x and y and can be negative. */
void gycanvas_box(GyCanvas * self, int x, int y, int w, int h);

/** Draws a box (the outline of a rectangle) on the canvas in the given
color, which is set as the draw color.  */
void gycanvas_boxcolor(GyCanvas * self, int x, int y, int w, int h, 
                        GyColor color);

/** Draws a slab (filled rectangle) on the canvas in the current draw color.
 W and h are the offsets from x and y and can be negative. */
...
enum GyTileFlags_ {
    GyTileWall = 1 << 0, // The tile is a wall.
...
#define GYTILEMAP_TILES_MAX 1024

/** A tileset is a set of tiles that are used by a tile map. */
struct GyTiledata_; 
typedef struct GyTiledata_ GyTiledata;
...
typedef int16_t GyTileindex;
typedef uint16_t GyTileinfo;
...
GyLayer * gylayer_new(GyTexture * texture, int gridwide, int gridhigh,
                           int tilewide, int tilehigh);
...
* to the given canvas. X and y may be negative. 
*/
...
/** Sets the tile at the given location to the given Tile index, 
